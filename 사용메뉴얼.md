...
| PyMuPDF | PDF 텍스트 추출 |
| watchdog | 폴더 실시간 감시 |
| requests | Gemini / CrossRef API 호출 |
...
# ── CrossRef 설정 ─────────────────────────────────
# 본인 이메일 입력 (CrossRef Polite Pool 우선 처리 레인 적용)
CROSSREF_MAILTO=본인이메일@example.com
```
...
| `reprocess_zotero_item.py` | Zotero 키 1개를 강제 재분석 (안전 백업 후 교체) |
| `batch_reprocess.py` | Placeholder/전체 노트 일괄 재처리 (안전 백업 후 교체) |
| `fill_new_sections.py` | 기존 노트의 빈 "연구 배경" / "분석결과" 섹션만 채움 |
| `regenerate_excerpts.py` | 기존 논문의 발췌(Excerpts) 섹션만 재생성 |
| `regenerate_excerpts_skipped.py` | 발췌 생성이 누락된 논문만 보완 |
| `regenerate_tags.py` | 기존 노트의 태그(hashtags)만 재생성 |
...
| `repair_zotero.py` | Zotero 메타데이터 불일치 복구 ⚠️ 아래 안전 모드 참고 |
| `obsidian_to_zotero.py` | Obsidian 마크다운 → Zotero 역방향 동기화 |

### 안전 모드 주의 — `repair_zotero.py`

이 스크립트는 **로컬 마크다운이 없는 Zotero 아이템을 "중복"으로 간주해 삭제**합니다. Vault가 불완전한 상태에서 실행하면 정상 라이브러리도 삭제될 수 있어, 다음 안전 장치가 적용되어 있습니다.

```bash
# 기본: 분석만 수행 (실제 변경 없음)
python repair_zotero.py

# 저자 수정 등 실제 변경 적용 (단, 삭제는 여전히 SKIP)
python repair_zotero.py --apply

# 1단계 중복 삭제까지 실제 수행 (위험)
python repair_zotero.py --apply --confirm-delete
```

- `--apply` 없이는 항상 dry-run으로 동작합니다
- `--apply`만 있으면 저자 수정 등은 적용되지만 1단계 삭제는 자동 SKIP
- `--confirm-delete`가 함께 있어야 삭제 단계가 활성화됩니다
- 삭제 대상이 **10개 이상**이면 `DELETE {개수}` 문자열을 정확히 타이핑해야 진행됨

### 재처리 안전장치 — `.bak` 파일

`batch_reprocess.py`와 `reprocess_zotero_item.py`는 기존 마크다운을 **즉시 삭제하지 않습니다**:

1. 원본을 `노트이름.md.bak`으로 이름 변경(백업)
2. AI 재분석 시도
3. 성공 시: `.bak` 파일 자동 정리
4. **실패 시: `.bak` 파일을 원본으로 자동 복원** (사용자 메모/발췌 보존)

작업 디렉토리에 `.bak` 파일이 남아있다면 이전 실행이 실패한 잔재이므로 내용을 확인 후 수동으로 처리하세요. (배포 패키지의 `.gitignore`는 `.bak` 파일을 자동 제외합니다.)
